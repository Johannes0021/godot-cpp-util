...
func write_gd_typed_signal_header(class_data_array: Array[ClassData], output_path: String) -> void:
	...
	for class_data in class_data_array:
		for sig in class_data.signals:
			...
			if (not forward_class_declarations.has(class_data.name)):
				forward_class_declarations.push_back(class_data.name)
	...
	file.store_line("")
	file.store_line("template<typename>")
	file.store_line("inline constexpr bool _helper_always_false_v = false;")
	file.store_line("")
	file.store_line("template<typename T>")
	file.store_line("struct GDTypedSignalTypeOf {")
	file.store_line(
		"    static_assert(_helper_always_false_v<T>, \"There is no GDTypedSignal of this type.\");"
	)
	file.store_line("};")
	file.store_line("")
	file.store_line("")
	...
	for class_data in class_data_array:
		...
		file.store_line("};")
		file.store_line("")
		file.store_line("template<>")
		file.store_line("struct GDTypedSignalTypeOf<godot::%s> {" % class_data.name)
		file.store_line("    using Type = %s;" % class_data.name)
		file.store_line("};")
		file.store_line("")
