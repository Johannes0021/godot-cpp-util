 ...
 * scripts/gen_gd_typed_signal.gd
 *
 * Usage Example:
 *
 * #pragma once
 ...
 *    struct Signal { // : public GDTypedSignal::Object {
 *         static constexpr TypedSignal<"my_signal"> my_signal{};
 *         static constexpr TypedSignal<"animation_finished", StringName> animation_finished{};
 *         static constexpr TypedSignal<"many_args", int, float, String, int> many_args{};
 *     };
 *
 * public:
...
struct typed_signal_helper_drop_last<0, Ts...> final {
...
struct typed_signal_helper_drop_last<N> final {
...
requires (N > 0)
struct typed_signal_helper_drop_last<N, T, Ts...> final {

private:
    static constexpr std::size_t tail_size = sizeof...(Ts);
...
struct typed_signal_helper_tuple_to_signal<Name, std::tuple<Ts...>> final {
...
private:
    const std::size_t m_unbind_arg_count = 0;



public:
    // Returns the number of arguments the signal expects.
    static constexpr std::size_t get_arg_count() {
        return sizeof...(Args);
    }



    // Compile-time check for correctness. get_arg_count() will never exceed INT64_MAX.
    static_assert(get_arg_count() <= INT64_MAX, "Too many arguments.");



    // Construct original TypedSignal.
    constexpr TypedSignal() = default;
     ...
     */
    constexpr std::size_t get_original_arg_count() const {
    ...
    constexpr auto unbind() const {
        ...
        using NewTypedSignal = typename typed_signal_helper_tuple_to_signal<Name, tuple>::type;

        return NewTypedSignal{m_unbind_arg_count + N};
    }
    ...
    Error connect(
        ...
        if (m_unbind_arg_count != 0) {
            ...
            return p_owner.connect(get_name(), p_static_binding.m_callable, p_flags);
        }
    }
    ...
    Error connect(
        ...
        if (m_unbind_arg_count != 0) {
            ...
            return p_owner.connect(get_name(), p_member_binding.m_callable, p_flags);
        }
    }
    ...
    Error emit(Object &p_owner, const Args &...p_args) const {
        return p_owner.emit_signal(get_name(), p_args...);
    }
...
private:
    ...
    TypedSignal(std::size_t p_unbind_arg_count)
